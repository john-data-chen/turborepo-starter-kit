...
export const useProjects = (
  boardId?: string | { _id: string; title: string }
) => {
  const id = boardId
...
export const useCreateProject = () => {
  ...
  return useMutation({
    ...
    onSuccess: (newProject) => {
      ...
      const boardId =
        typeof newProject.board === 'string'
          ? newProject.board
          : newProject.board?._id;
...
export const useUpdateProject = () => {
  ...
  return useMutation({
    mutationFn: async ({
      id,
      ...updates
    }: { id: string } & Omit<UpdateProjectInput, 'owner'>) => {
    ...
    onSuccess: (updatedProject) => {
      ...
      const boardId =
        typeof updatedProject.board === 'string'
          ? updatedProject.board
          : updatedProject.board?._id;
