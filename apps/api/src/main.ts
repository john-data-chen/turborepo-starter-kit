...
async function bootstrap() {
  ...
  const corsMiddleware = (req: any, res: any, next: any) => {
    ...
    if (requestMethod === 'OPTIONS') {
      console.log(`[${requestId}] [CORS] Handling preflight request for origin: ${origin}`)
      ...
      console.log(`[${requestId}] [CORS] Preflight response headers set:`, {
        ...
        'Access-Control-Allow-Headers': 'Content-Type,Accept,Authorization,X-Requested-With,X-XSRF-TOKEN,Cookie'
      })
    ...
    if (origin) {
      ...
      if (isAllowed) {
        ...
        res.header('Access-Control-Allow-Origin', origin)
        res.header('Access-Control-Allow-Credentials', 'true')
        console.log(`[${requestId}] [CORS] Regular request headers set for ${origin}`)
      } else {
  ...
  console.log('CORS Configuration:', {
    allowedOrigins: allowedOrigins.map((origin) => (origin instanceof RegExp ? origin.toString() : origin)),
